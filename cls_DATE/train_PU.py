...
def create_data_loader():
    ...
    Xvaild,_,Yvaild,_ = splitTrainTestSet(Xtest, ytest, 0.90)
    ...
    for r in range(H):
        for c in range(W):
            ...
            if(target == 0):
                ...
                prediction[r][c] = label_pred[ind]+2
                
                ind += 1
  
    print(prediction)
    spectral.save_rgb("predictions_dat_pu.png", prediction.astype(int), colors=spectral.spy_colors)
...
#    margin = int((patch_size - 1) / 2)
    ...
    spectral.save_rgb("predictions_pU.png", prediction.astype(int), colors=spectral.spy_colors)
    return prediction
...
if __name__ == '__main__':
    ...
    net, device = train(train_loader,vaild_loader,dropout=dropout,emb_dropout=emb_dropout,epochs=70)
    # 只保存模型参数
